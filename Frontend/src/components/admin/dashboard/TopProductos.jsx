  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-8">
        <div>
          <Title level={4} className="!m-0 !font-black dark:!text-white">Best Sellers</Title>
      ...
      {topProductos.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center py-10 opacity-50">
          ...
          <Text className="!font-bold dark:!text-slate-300">Sin datos históricos</Text>
        </div>
      ) : (
        <div className="space-y-4 flex-1">
          {topProductos.slice(0, 5).map((item, index) => {
            ...
            return (
              ...
              >
                <div 
                  className="w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs shrink-0 transition-transform group-hover:scale-110"
                ...
                <div className="flex-1 min-w-0">
                  <Text className="!font-bold block truncate !text-sm dark:!text-slate-200 group-hover:text-blue-500 transition-colors">
                ...
                <div className="text-right">
                  <div className="flex items-center gap-1 justify-end">
                    ...
                    <Text className="!font-black !text-sm dark:!text-slate-200">{item._sum.cantidad}</Text>
                  </div>
                  <Text className="!text-[10px] !text-slate-400 dark:!text-slate-500 !font-bold uppercase tracking-tighter">UNIDADES</Text>
                </div>
              </div>
            );
          })}
      ...
      <div className="mt-8 pt-4 border-t border-slate-100 dark:border-slate-700/50">
        <button className="w-full py-2 text-blue-500 dark:text-blue-400 font-bold text-sm hover:underline">Ver reporte detallado</button>
      </div>
    </div>
  );
};
