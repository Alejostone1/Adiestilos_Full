    ...
    if (!hasSubitems) {
      return (
        ...
        >
          ...
          {!sidebarCollapsed && (
            <span className={`font-semibold text-sm tracking-tight ${isItemActive ? 'opacity-100' : 'opacity-80 group-hover:opacity-100'}`}>
              {section.title}
            </span>
          )}
          ...
          {sidebarCollapsed && (
            <div className="absolute left-full ml-4 px-4 py-2 bg-slate-900 text-white text-[11px] font-semibold uppercase tracking-wide rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-50 shadow-2xl">
              {section.title}
            </div>
          )}
        </Link>
          ...
          <div className={`flex items-center gap-4 transition-all duration-500 ${sidebarCollapsed ? 'justify-center' : ''}`}>
            ...
            {!sidebarCollapsed && (
              <div className="flex-1 animate-in fade-in slide-in-from-left-4 duration-700">
                <h1 className="text-xl font-semibold tracking-tighter text-gradient dark:text-white !m-0 leading-tight">
                  Adi Estilos
                </h1>
